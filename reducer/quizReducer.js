...
import {
  CORRECT, WRONG, DISPLAYED, PLAYED, NEXT, INCREASE_SCORE, DECREASE_SCORE, LOAD_QUESTION,
  SKIP, ACTIVE, TIME_OUT, PLAY_AGAIN, ANSWER, CORRECT_ANS, SET_TIME, START_GAME_ERR, NO_POINTS,
  PLAYED_CURRENT, PLAYED_PREV, RESET_PLAYED_CURRENT, SET_OVERLAY, LOADING_QUIZ, START_GAME,
} from '../actions/quiz';
      ...
      return produce(state, (draft) => {
        ...
        if (Object.entries(payload).length) {
          ...
          if (payload.easy.length > 0) {
            level = 'easy';
          } else if (payload.moderate.length > 0) {
            level = 'moderate';
          } else {
            level = 'difficult';
          }
        ...
        if (questions[level].length) {
          ...
          if (level === 'easy') {
            if (questions.moderate.length) {
              level = 'moderate';
            } else if (questions.difficult.length) {
              level = 'difficult';
            }
