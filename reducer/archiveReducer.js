...
export default function archiveReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case UNARCHIVE: {
      return produce(state, (draft) => {
        draft.archive = isJson(state.archive.slice()).map((item, i) => {
          const index = item.value.indexOf(action.payload.item.id);
          if (index !== -1) {
    ...
    case SEARCH_ARCHIVE: {
      return produce(state, (draft) => {
        const textArr = action.payload.trim().split(' ');
        const newArchive = state.archive.filter((element, i) => {
    ...
    case PREV_SEARCH: {
      return produce(state, (draft) => {
        const textArr = action.payload.trim().split(' ');
        const newSearchText = state.searchText.filter((element, i) => {
